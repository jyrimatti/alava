...
import Foundation (($),Functor,fmap,pure,(<*>),(>>=),(<>),Applicative,foldl,Monad,String,Show,show,Bool(True,False),(==),(&&),Maybe(Just,Nothing),(<),(>),Ordering(LT,EQ,GT),error,toList)
...
import Environment (Env,getSourceLocation,lookupDef,extendCtx)
...
data Result a = Success a | Failure [(Error,SourcePos)] deriving Show
...
err :: Error -> Result a
err e = Failure $ [(e, getSourceLocation (getEnv e))]

with :: [(Error, SourcePos)] -> Result t -> Result a
with es (Failure e) = Failure $ es <> e
with _ _ = error "Should not be here"

equate :: HasCallStack => Env -> Term -> Term -> Bool
equate env t1 t2 = let
  n1 = whnf' env False t1  
  n2 = whnf' env False t2
  o (Def x1 _) (Def x2 _) = if x1 < x2 then LT else if x1 > x2 then GT else EQ
  recEquate x _ = case lookupDef env x of 
           Just d -> equate env d n2
           Nothing -> error $ toList $ "Expected: " <> show n2 <> " , found: " <> show (Var x)
  equateMaybe _ Nothing Nothing     = True
  equateMaybe env (Just a) (Just b) = equate env a b
  equateMaybe _ _ _                 = False
  in case (n1, n2) of
    (Type, Type)                     -> True
    (Var x, Var y)                   -> x == y
    (Lam _ _ b1, Lam _ _ b2)         -> equate env b1 b2
    (App a1 a2, App b1 b2)           -> equate env a1 b1 && equate env a2 b2
    (Pi _ tyA1 tyB1, Pi _ tyA2 tyB2) -> equate env tyA1 tyA2 && equate env tyB1 tyB2
    (Ann _ at1 _, at2)               -> equate env at1 at2
    (at1, Ann _ at2 _)               -> equate env at1 at2
    (Paren at1, at2)                 -> equate env at1 at2
    (at1, Paren at2)                 -> equate env at1 at2
    (Let xs1 body1, Let xs2 body2)   -> equate env body1 body2 && (all (\(x1,x2) -> equate env x1 x2) $ zip (sortBy o xs1) (sortBy o xs2))
    (Sigma _ tyA1 tyB1, Sigma _ tyA2 tyB2) -> equateMaybe env tyA1 tyA2 && equateMaybe env tyB1 tyB2
    (Prod a1 a2 a3, Prod b1 b2 b3)   -> equateMaybe env a1 b1 && equateMaybe env a2 b2 && equateMaybe env a3 b3
...
ensurePi :: HasCallStack => Env -> Type -> Result (Maybe TName, Type, Type)
ensurePi env ty = case whnf env ty of 
    Whnf (Pi mname tyA tyB) -> pure (mname, tyA, tyB)
    Whnf nf -> err $ ExpectedFunctionType env ty nf
...
whnf' env b (App t1 t2) = case whnf' env b t1 of 
    (Lam x _ body) -> whnf' (extendCtx (Def x t2) env) b body
    nf               -> App nf t2
...
whnf' _ _ t@(Ann _ _ _) = error $ toList $ "Unexpected arg to whnf: " <> show t
whnf' _ _ t@(Paren _)   = error $ toList $ "Unexpected arg to whnf: " <> show t
whnf' _ _ t@(Pos _ _)   = error $ toList $ "Unexpected arg to whnf: " <> show t
